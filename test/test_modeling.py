"""
unittest for ad_hoc/modeling.py
"""
...
from ad_hoc.modeling import add_prefix_to_param, simple_pipeline_cv
from ad_hoc.modeling import cv_results_summary, pick_the_last_estimator
from ad_hoc.modeling import show_coefficients, show_feature_importance
from ad_hoc.modeling import recover_label, ROCCurve
from ad_hoc.utilities import load_boston, load_iris, load_breast_cancer
...
class ModelingTest(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.breast_cancer_plr = GridSearchCV(LogisticRegression(solver="liblinear"), param_grid={"C": [0.1, 1]}, cv=3)
    ...
    def test_add_prefix_to_param(self):
        ...
        pref = "YourPrefix"
        before = {"param_1": [1, 2, 3], "param_2": [5, 9, 1]}
    ...
    def test_simple_pipeline_cv(self):
        ...
        cv = 11
        param_grid = {"C": [2, 3, 5]}
        model = simple_pipeline_cv(name=name, model=plr, param_grid=param_grid, scaler=MinMaxScaler(), cv=cv)
        ...
        self.assertTrue(isinstance(pipeline.steps[0][1], MinMaxScaler))
        self.assertEqual(name, pipeline.steps[1][0])
        self.assertTrue(isinstance(pipeline.steps[1][1], LogisticRegression))
    ...
    def test_cv_results_summary(self):
        ...
        cols = ["mean_test_score", "std_test_score", "test_CI_low", "test_CI_high"]
    ...
    def test_show_coefficients_classification(self):
        ...
        reg_coef = show_coefficients(self.iris_plr, self.iris_X.columns)
    ...
    def test_show_coefficients_regression(self):
        reg_coef = show_coefficients(self.boston_enet, self.boston_X.columns)
    ...
    def test_show_coefficients_binary_classification(self):
        reg_coef = show_coefficients(self.breast_cancer_plr, self.breast_cancer_X.columns)
    ...
    def test_show_feature_importance(self):
        ...
        importance = show_feature_importance(estimator=self.boston_tree, columns=self.boston_X.columns)
        ...
        with self.assertRaises(AttributeError):
            show_feature_importance(estimator=self.iris_plr, columns=self.iris_X.columns)
    ...
    def test_show_feature_importance_model(self):
        ...
        importance = show_feature_importance(estimator=model, columns=self.boston_X.columns)
    ...
    def test_recover_label(self):
        ...
        ## Case 1) without sep
        dg = recover_label(
            data=Y, field2columns={"SPECIES": ["setosa", "virginica"]}, sep=None, other="OTHER", inplace=False
        )
        ...
        dg = recover_label(
            data=Y, field2columns={"FIELD": ["FIELD|setosa", "FIELD|virginica"]}, sep="|", other="OTHER", inplace=False
        )
...
class TestROCCurve(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        plr.fit(X, y)
        Y = pd.DataFrame(plr.predict_proba(X), columns=plr.classes_)
    ...
    def setUp(self) -> None:
        np.random.seed(1)
        self.roc = ROCCurve(y_true=self.y_true, y_score=self.y_score, pos_label="malignant")
    ...
    def test_ROCCurve(self):
        ...
        ## scores attribute
        cols = ["recall", "precision", "fp_rate", "accuracy", "f1_score", "n_pos_pred"]
    ...
    def test_predict_thru_threshold(self):
        ...
        y_pred = pd.Series(y_pred, index=self.y_true.index).map({0: "benign", 1: "malignant"})
    ...
    def test_get_scores_thru_threshold(self):
        ...
        index = ["threshold", "recall", "precision", "accuracy", "f1_score", "n_pos_pred", "fp_rate"]

        self.assertEqual(index, list(s.index))
        self.assertEqual((self.y_true == self.y_pred).mean(), s["accuracy"])
    ...
    def test_optimize_expected_value(self):
        df_scoring = self.roc.optimize_expected_value(proportion_positive=self.roc.y_true.mean(), scaling=True)
        ...
        self.assertAlmostEqual(last_row["expected (score)"], last_row["expected (random)"])

        self.assertEqual(self.roc.y_true.sum(), last_row["n_true"])

        ## check if it is error-free
        self.roc.show_expected_value(proportion_positive=self.roc.y_true.mean(), scaling=True)
